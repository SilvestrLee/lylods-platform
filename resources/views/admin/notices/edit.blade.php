    ...
    <x-slot name="header">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <h2 class="mt-1 font-serif text-2xl font-bold text-[#07172f]">Edit Notice</h2>
            </div>
    ...
    <x-admin-dashboard-shell>
        ...
        <div class="overflow-hidden rounded-[2rem] bg-white shadow-sm ring-1 ring-[#e6e8ee]">
            <div class="border-b border-[#e6e8ee] p-6">
                <p class="text-sm font-bold uppercase tracking-[0.22em] text-[#123f8c]">Existing Notice</p>
                <h1 class="mt-2 text-2xl font-bold text-[#07172f]">Update Investor Notice</h1>
            ...
            <form method="POST"
                ...
                <div class="grid gap-6">
                    ...
                    <div>
                        ...
                        <textarea id="body" name="body" rows="8" required
                                  class="mt-2 block w-full rounded-2xl border-[#d0d5dd] @error('body') border-red-400 ring-1 ring-red-400 @enderror shadow-sm focus:border-[#123f8c] focus:ring-[#123f8c]">{{ old('body', $notice->body) }}</textarea>
                        @error('body')
                            <p class="mt-2 text-sm font-semibold text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <label class="flex items-start gap-3 rounded-2xl bg-[#f8fafc] p-4">
                        ...
                        <span>
                            ...
                            <span class="mt-1 block text-sm leading-6 text-[#667085]">Published notices will appear on investor dashboards. Uncheck to save as draft.</span>
                        </span>
                    </label>

                    <div class="flex items-center gap-4 rounded-2xl bg-[#f8fafc] p-5">
                        <div>
                            <p class="text-sm font-bold text-[#07172f]">Notice Status</p>
                            <p class="mt-1 text-sm text-[#667085]">
                                Current: <x-status-badge :status="$notice->is_published ? 'published' : 'draft'" />
                            </p>
                        </div>
                        @if ($notice->published_at)
                            <div class="ml-auto text-right">
                                <p class="text-xs text-[#98a2b3]">Published date</p>
                                <p class="text-sm font-semibold text-[#07172f]">{{ $notice->published_at->format('M d, Y') }}</p>
                            </div>
                        @endif
                    </div>
                </div>
                ...
                <div class="mt-8 flex flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-end">
                    ...
                    <button type="submit"
                        ...
                        <span x-text="loading ? 'Saving…' : 'Update Notice'">Update Notice</span>
                    </button>
                </div>
            </form>
        </div>
