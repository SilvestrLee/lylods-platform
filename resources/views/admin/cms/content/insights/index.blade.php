    ...
    <div class="rounded-[2rem] bg-white p-8 shadow-sm">
        <div class="flex items-center justify-between gap-4">
            <div>
                ...
                <h1 class="mt-1 text-2xl font-bold text-[#07172f]">Insights</h1>
            </div>
            <a href="{{ route('admin.cms.content.insights.create') }}"
            ...
            <table class="w-full text-sm">
                <thead class="bg-[#f7f3ea]">
                    <tr>
                        ...
                        <th class="hidden px-5 py-3.5 text-left text-xs font-bold uppercase tracking-wider text-[#667085] md:table-cell">Category</th>
                ...
                <tbody class="divide-y divide-[#e6e8ee]">
                    @forelse ($insights as $insight)
                        <tr class="hover:bg-[#f7f3ea]/60">
                            ...
                            <td class="hidden px-5 py-4 text-[#667085] md:table-cell">{{ $insight->category ?? '—' }}</td>
                            <td class="px-5 py-4">
                            ...
                            <td class="hidden px-5 py-4 text-[#667085] lg:table-cell">{{ $insight->published_at?->format('d M Y') ?? '—' }}</td>
                            <td class="px-5 py-4 text-right">
                                <a href="{{ route('admin.cms.content.insights.edit', $insight) }}"
                                   class="rounded-xl bg-[#07172f] px-3.5 py-1.5 text-xs font-bold text-white hover:bg-[#0d2144]">Edit</a>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="5" class="px-5 py-12 text-center text-sm text-[#667085]">No insights yet. Add your first above.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
