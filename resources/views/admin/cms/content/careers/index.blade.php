    ...
    <div class="rounded-[2rem] bg-white p-8 shadow-sm">
        <div class="flex items-center justify-between gap-4">
            <div>
                ...
                <h1 class="mt-1 text-2xl font-bold text-[#07172f]">Career Opportunities</h1>
            </div>
            <a href="{{ route('admin.cms.content.careers.create') }}"
            ...
            <table class="w-full text-sm">
                <thead class="bg-[#f7f3ea]">
                    <tr>
                        ...
                        <th class="hidden px-5 py-3.5 text-left text-xs font-bold uppercase tracking-wider text-[#667085] md:table-cell">Type</th>
                        <th class="hidden px-5 py-3.5 text-left text-xs font-bold uppercase tracking-wider text-[#667085] md:table-cell">Location</th>
                        <th class="px-5 py-3.5 text-left text-xs font-bold uppercase tracking-wider text-[#667085]">Status</th>
                        <th class="hidden px-5 py-3.5 text-left text-xs font-bold uppercase tracking-wider text-[#667085] lg:table-cell">Closes</th>
                ...
                <tbody class="divide-y divide-[#e6e8ee]">
                    @forelse ($careers as $career)
                        <tr class="hover:bg-[#f7f3ea]/60">
                            <td class="px-5 py-4 font-medium text-[#07172f]">{{ $career->title }}</td>
                            <td class="hidden px-5 py-4 text-[#667085] md:table-cell capitalize">{{ str_replace('-', ' ', $career->type) }}</td>
                            <td class="hidden px-5 py-4 text-[#667085] md:table-cell">{{ $career->location ?? '—' }}</td>
                            <td class="px-5 py-4">
                            ...
                            <td class="hidden px-5 py-4 text-[#667085] lg:table-cell">{{ $career->closing_date?->format('d M Y') ?? '—' }}</td>
                            <td class="px-5 py-4 text-right">
                                <a href="{{ route('admin.cms.content.careers.edit', $career) }}"
                                   class="rounded-xl bg-[#07172f] px-3.5 py-1.5 text-xs font-bold text-white hover:bg-[#0d2144]">Edit</a>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="6" class="px-5 py-12 text-center text-sm text-[#667085]">No career opportunities yet. Add your first above.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
