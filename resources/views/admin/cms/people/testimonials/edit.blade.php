<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <h2 class="mt-1 font-serif text-2xl font-bold text-[#07172f]">Edit — {{ $testimonial->client_name }}</h2>
            </div>
    ...
    <x-admin-dashboard-shell>
        <x-slot name="breadcrumbs">
            <x-breadcrumbs :crumbs="[['label' => 'CMS'], ['label' => 'People'], ['label' => 'Testimonials', 'url' => route('admin.cms.people.testimonials.index')], ['label' => $testimonial->client_name]]" />
        </x-slot>
        ...
        <form method="POST" action="{{ route('admin.cms.people.testimonials.update', $testimonial) }}">
            @csrf
            @method('PATCH')

            <div class="overflow-hidden rounded-[2rem] bg-white shadow-sm ring-1 ring-[#e6e8ee]">
                <div class="border-b border-[#e6e8ee] p-6">
                    <h1 class="text-xl font-bold text-[#07172f]">Client Testimonial</h1>
                </div>

                <div class="space-y-6 p-6">

                    <div>
                        <label class="block text-sm font-semibold text-[#07172f]">Quote <span class="text-red-500">*</span></label>
                        <textarea name="quote" rows="4" required
                                  class="mt-2 w-full rounded-2xl border border-[#d0d5dd] px-4 py-3 text-sm text-[#07172f] shadow-sm focus:border-[#123f8c] focus:outline-none focus:ring-1 focus:ring-[#123f8c] @error('quote') border-red-400 @enderror">{{ old('quote', $testimonial->quote) }}</textarea>
                        @error('quote')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror
                    </div>

                    <div class="grid gap-6 sm:grid-cols-2">
                        <div>
                            <label class="block text-sm font-semibold text-[#07172f]">Client Name <span class="text-red-500">*</span></label>
                            <input type="text" name="client_name" value="{{ old('client_name', $testimonial->client_name) }}" required
                                   class="mt-2 w-full rounded-2xl border border-[#d0d5dd] px-4 py-3 text-sm text-[#07172f] shadow-sm focus:border-[#123f8c] focus:outline-none focus:ring-1 focus:ring-[#123f8c] @error('client_name') border-red-400 @enderror">
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-[#07172f]">Organisation / Company</label>
                        <input type="text" name="organisation" value="{{ old('organisation', $testimonial->organisation) }}"
                               class="mt-2 w-full rounded-2xl border border-[#d0d5dd] px-4 py-3 text-sm text-[#07172f] shadow-sm focus:border-[#123f8c] focus:outline-none focus:ring-1 focus:ring-[#123f8c]">
                    </div>
                    ...
                    <div class="grid gap-6 sm:grid-cols-2">
                        <div>
                            ...
                            <input type="number" name="display_order" value="{{ old('display_order', $testimonial->display_order) }}" min="0"
                                   class="mt-2 w-full rounded-2xl border border-[#d0d5dd] px-4 py-3 text-sm text-[#07172f] shadow-sm focus:border-[#123f8c] focus:outline-none focus:ring-1 focus:ring-[#123f8c]">
                        ...
                        <div class="space-y-3 pt-8">
                            <div class="flex items-center gap-3">
                                ...
                                <label for="featured" class="text-sm font-semibold text-[#07172f]">Featured testimonial</label>
                            </div>
                            <div class="flex items-center gap-3">
                ...
                <div class="flex items-center justify-between border-t border-[#e6e8ee] bg-[#f8fafc] px-6 py-5">
                    ...
                    <form method="POST" action="{{ route('admin.cms.people.testimonials.destroy', $testimonial) }}" onsubmit="return confirm('Remove this testimonial?')">
                        @csrf
                        @method('DELETE')
