<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <h2 class="mt-1 font-serif text-2xl font-bold text-[#07172f]">Add Team Member</h2>
            </div>
    ...
    <x-admin-dashboard-shell>
        <x-slot name="breadcrumbs">
            <x-breadcrumbs :crumbs="[['label' => 'CMS'], ['label' => 'People'], ['label' => 'Team', 'url' => route('admin.cms.people.team.index')], ['label' => 'Add Member']]" />
        </x-slot>

        <form method="POST" action="{{ route('admin.cms.people.team.store') }}">
            @csrf
                ...
                <div class="space-y-6 p-6">
                    ...
                    <div>
                        ...
                        <textarea name="bio" rows="4"
                                  class="mt-2 w-full rounded-2xl border border-[#d0d5dd] px-4 py-3 text-sm text-[#07172f] shadow-sm focus:border-[#123f8c] focus:outline-none focus:ring-1 focus:ring-[#123f8c]">{{ old('bio') }}</textarea>
                    </div>
