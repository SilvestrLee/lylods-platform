@php $steps = $page->aboutHowWeWorkSteps->values(); @endphp
            ...
            <div class="rounded-2xl border border-[#e6e8ee] p-5">
                ...
                <div class="space-y-4">
                    <x-admin.field label="Title">
                        <input type="text" name="how_we_work_steps[{{ $i }}][title]" value="{{ old("how_we_work_steps.$i.title", $step?->title) }}"
                    ...
                    <x-admin.field label="Description">
                        <textarea name="how_we_work_steps[{{ $i }}][description]" rows="3"
                                  class="w-full rounded-2xl border border-[#d0d5dd] px-4 py-3 text-sm text-[#07172f] shadow-sm focus:border-[#123f8c] focus:outline-none focus:ring-1 focus:ring-[#123f8c]">{{ old("how_we_work_steps.$i.description", $step?->description) }}</textarea>
                    </x-admin.field>
                    <label class="flex items-center gap-2 text-xs font-semibold text-[#07172f]">
                        <input type="checkbox" name="how_we_work_steps[{{ $i }}][visibility]" value="1"
                               {{ old("how_we_work_steps.$i.visibility", $step?->visibility ?? true) ? 'checked' : '' }}
