<x-app-layout>
    <x-slot name="header">
        <div>
            ...
            <h2 class="mt-1 font-serif text-2xl font-bold text-[#07172f]">Add Footer Link</h2>
        </div>
    </x-slot>

    <x-admin-dashboard-shell>
        <x-slot name="breadcrumbs">
            <x-breadcrumbs :crumbs="[['label' => 'CMS'], ['label' => 'Footer'], ['label' => 'Footer Links', 'url' => route('admin.cms.footer.links.index')], ['label' => 'Add']]" />
        </x-slot>

        <div class="overflow-hidden rounded-[2rem] bg-white shadow-sm ring-1 ring-[#e6e8ee]">
            <div class="border-b border-[#e6e8ee] p-6">
                <h1 class="text-xl font-bold text-[#07172f]">New Footer Link</h1>
            </div>
            <form method="POST" action="{{ route('admin.cms.footer.links.store') }}" class="space-y-6 p-6">
                @csrf
                ...
                <div class="grid gap-6 sm:grid-cols-2">
                    <div>
                        ...
                        <select name="group" class="mt-1.5 w-full rounded-xl border border-[#e6e8ee] bg-white px-4 py-2.5 text-sm focus:border-[#c9a24d] focus:outline-none focus:ring-1 focus:ring-[#c9a24d]">
                            @foreach (['company' => 'Company', 'services' => 'Services', 'industries' => 'Industries', 'insights' => 'Insights', 'support' => 'Support', 'portal' => 'Portal'] as $val => $lbl)
                ...
                <div>
                    ...
                    <input type="text" name="url" value="{{ old('url') }}" placeholder="/about or https://..."
                           class="mt-1.5 w-full rounded-xl border border-[#e6e8ee] px-4 py-2.5 text-sm focus:border-[#c9a24d] focus:outline-none focus:ring-1 focus:ring-[#c9a24d]">
