<x-app-layout>
    <x-slot name="header">
        <div>
            ...
            <h2 class="mt-1 font-serif text-2xl font-bold text-[#07172f]">Edit Social Link</h2>
        </div>
    </x-slot>

    <x-admin-dashboard-shell>
        <x-slot name="breadcrumbs">
            <x-breadcrumbs :crumbs="[['label' => 'CMS'], ['label' => 'Footer'], ['label' => 'Social Links', 'url' => route('admin.cms.footer.social.index')], ['label' => ucfirst($socialLink->platform)]]" />
        </x-slot>
        ...
        <div class="overflow-hidden rounded-[2rem] bg-white shadow-sm ring-1 ring-[#e6e8ee]">
            <div class="border-b border-[#e6e8ee] p-6">
                <h1 class="text-xl font-bold text-[#07172f] capitalize">{{ $socialLink->platform }}</h1>
            </div>
            <form method="POST" action="{{ route('admin.cms.footer.social.update', $socialLink) }}" class="space-y-6 p-6">
                @csrf @method('PATCH')

                <div class="grid gap-6 sm:grid-cols-2">
                    <div>
                        <label class="block text-sm font-semibold text-[#07172f]">Platform</label>
                        <select name="platform" class="mt-1.5 w-full rounded-xl border border-[#e6e8ee] bg-white px-4 py-2.5 text-sm focus:border-[#c9a24d] focus:outline-none focus:ring-1 focus:ring-[#c9a24d]">
                            @foreach (['linkedin', 'x', 'facebook', 'instagram', 'youtube'] as $platform)
                                <option value="{{ $platform }}" {{ old('platform', $socialLink->platform) === $platform ? 'selected' : '' }}>{{ ucfirst($platform) }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="number" name="display_order" min="0" value="{{ old('display_order', $socialLink->display_order) }}"
                ...
                <div>
                    <label class="block text-sm font-semibold text-[#07172f]">Profile URL</label>
                    <input type="text" name="url" value="{{ old('url', $socialLink->url) }}"
                           class="mt-1.5 w-full rounded-xl border border-[#e6e8ee] px-4 py-2.5 text-sm focus:border-[#c9a24d] focus:outline-none focus:ring-1 focus:ring-[#c9a24d]">
