<x-app-layout>
    <x-slot name="header">
        <div>
            ...
            <h2 class="mt-1 font-serif text-2xl font-bold text-[#07172f]">Add Social Link</h2>
        </div>
    </x-slot>

    <x-admin-dashboard-shell>
        <x-slot name="breadcrumbs">
            <x-breadcrumbs :crumbs="[['label' => 'CMS'], ['label' => 'Footer'], ['label' => 'Social Links', 'url' => route('admin.cms.footer.social.index')], ['label' => 'Add']]" />
        </x-slot>

        <div class="overflow-hidden rounded-[2rem] bg-white shadow-sm ring-1 ring-[#e6e8ee]">
            <div class="border-b border-[#e6e8ee] p-6">
                <h1 class="text-xl font-bold text-[#07172f]">New Social Link</h1>
            </div>
            <form method="POST" action="{{ route('admin.cms.footer.social.store') }}" class="space-y-6 p-6">
                @csrf
