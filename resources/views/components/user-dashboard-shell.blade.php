    ...
    <div class="mx-auto max-w-7xl px-4 pt-5 pb-0 sm:px-6 lg:pt-7">
        <div class="grid gap-5 lg:grid-cols-[268px_minmax(0,1fr)]">
            ...
            <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
                ...
                <h3 class="mt-2 text-lg font-bold text-white">Investor Area</h3>

                <nav class="mt-6 flex-1 space-y-0.5 text-sm font-medium" aria-label="Investor navigation">

                    {{-- Overview --}}
                    <a href="{{ route('dashboard') }}"
                       class="flex items-center gap-3 rounded-xl px-3.5 py-2.5 transition {{ request()->routeIs('dashboard') ? 'bg-white/15 text-white' : 'text-slate-300 hover:bg-white/10 hover:text-white' }}"
                    ...
                    <a href="{{ route('dashboard') }}#investments"
                       class="flex items-center gap-3 rounded-xl px-3.5 py-2.5 text-slate-300 transition hover:bg-white/10 hover:text-white">
                        <svg class="h-4 w-4 shrink-0" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24">
                    ...
                    <a href="{{ route('dashboard.notices') }}"
                       class="flex items-center gap-3 rounded-xl px-3.5 py-2.5 transition {{ request()->routeIs('dashboard.notices*') ? 'bg-white/15 text-white' : 'text-slate-300 hover:bg-white/10 hover:text-white' }}">
                        <svg class="h-4 w-4 shrink-0" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24">
                    ...
                    <span class="flex cursor-not-allowed items-center gap-3 rounded-xl px-3.5 py-2.5 text-slate-500 opacity-50">
                        <svg class="h-4 w-4 shrink-0" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24">
                    ...
                    <a href="{{ route('profile.edit') }}"
                       class="flex items-center gap-3 rounded-xl px-3.5 py-2.5 transition {{ request()->routeIs('profile.edit') ? 'bg-white/15 text-white' : 'text-slate-300 hover:bg-white/10 hover:text-white' }}">
                        <svg class="h-4 w-4 shrink-0" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24">
                ...
                @endphp
                <div class="mt-6 flex items-center gap-3 rounded-xl bg-white/10 px-4 py-3">
                    <span class="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[#c9a24d] text-xs font-bold text-[#07172f]">
                        {{ $investorInitials }}
                    </span>
                    <div class="min-w-0">
                        <p class="truncate text-sm font-semibold text-white">{{ Auth::user()->name }}</p>
                        <p class="truncate text-xs text-slate-400">Investor Account</p>
                    </div>
                </div>
                @endauth
            </aside>

            <main class="min-w-0 space-y-5 pb-8">
